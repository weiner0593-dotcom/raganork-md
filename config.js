...
const baseConfig = {
  ...
  ALIVE:
    process.env.ALIVE ||
    "⚡ JocelinBot is running smoothly!",
  ...
  RG: process.env.RG || "233533416608-1632403322@g.us,120363116963909366@g.us",
  BOT_INFO: process.env.BOT_INFO || "DemigodBot",
  RBG_KEY: process.env.RBG_KEY || "",
  ALLOWED: process.env.ALLOWED || "91,94,233",
  NOT_ALLOWED: process.env.NOT_ALLOWED || "852",
  CHATBOT: process.env.CHATBOT || "off",
  HANDLERS: process.env.HANDLERS || ".,",
  STICKER_DATA: process.env.STICKER_DATA || "Demigod",
  BOT_NAME: process.env.BOT_NAME || "Demigod",
  AUDIO_DATA:
...
module.exports = config;  const isWriteQuery = (sql) => {
    if (!sql || typeof sql !== "string") return true; // default to safe mode
    const normalizedSql = sql.trim().toUpperCase();
    return (
      normalizedSql.startsWith("INSERT") ||
